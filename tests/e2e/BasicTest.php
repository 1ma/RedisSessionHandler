...
class BasicTest extends EndToEndTestCase
{
     ...
     * with the new session ID, its body should be '1', and the
     * APCu cache should have exactly one entry.
     */
     ...
     * both their bodies must be '1'. The APCu cache should have
     * exactly two entries.
     */
    public function testUnrelatedAnonymousRequests()
    {
        ...
        $secondResponse = $this->client->send(
            new Request('GET', '/visit-counter.php', [], null, '1.0')
        );
     ...
     * should be '2'. The APCu cache should have exactly one entry.
     */
     ...
     * a newly generated ID and its body should be '1'. The APCu cache
     * should have exactly one entry, and its key should not be 'madeupkey'.
     */
